...
async def confirm_sending_token_group(
    ...
    await confirm_hex(
        ...
        title="Confirm transaction",
        subtitle="Policy id:",
        data=hexlify(token_group.policy_id).decode(),
        br_code=ButtonRequestType.Other,
    )
...
async def confirm_sending_token_ascii(
    ...
    await confirm_properties(
        ...
        props=[
            (
                "Asset #%s name (ASCII):" % token_number,
                token.asset_name_bytes.decode("ascii"),
            ),
...
async def confirm_sending_token_hex(
    ...
    await confirm_properties(
        ...
        props=[
            (
                "Asset #%s name (hex):" % token_number,
                hexlify(token.asset_name_bytes).decode(),
            ),
            ("Amount sent:", format_amount(token.amount, 0)),
        ],
        br_code=ButtonRequestType.Other,
...
async def confirm_transaction(
    ...
    if has_metadata:
        props.append(("\nTransaction contains metadata.", None))
...
async def confirm_stake_pool_owners(
    ...
    props: List[Tuple[str, Optional[str]]] = []
